...
class STTNode(Node):
    def __init__(self):
        ...
        self.CHANNELS = 1
        self.RATE = 48000        # Whisper 권장 샘플링 레이트
        self.CHUNK = 1024        # 버퍼 크기
        self.RECORD_SECONDS = 5  # 60초 단위로 오디오를 처리
    ...
    def record_audio_segment(self, seconds):
        ...
        self.get_logger().info("✅ 오디오를 'debug_audio.wav' 파일로 저장했습니다.")
    ...
    def run(self):
        ...
        while rclpy.ok():
            if not activated:
                ...
                self.get_logger().info(f"Whisper가 인식한 전체 텍스트: '{recognized_text}'")
